  ...
  const exportCSV = () => {
    const rows = [
      ...
      ...filteredInvoices.map(inv => [
        ...
        inv.createdAt ? inv.createdAt.toLocaleDateString() : '',
    ...
    <div className="dashboard-container">
      ...
      <main className="main-content">
        <header className="main-header">
          <h1>Reports</h1>
          <p>Business insights, analytics, and financial summaries.</p>
        </header>
        <div style={{ marginBottom: 30, display: 'flex', gap: 20, flexWrap: 'wrap', alignItems: 'center' }}>
          <div>
            ...
            <button onClick={exportCSV} style={{ padding: '8px 18px', background: '#6a11cb', color: 'white', border: 'none', borderRadius: 5, fontWeight: 600, fontSize: 13, cursor: 'pointer', transition: 'background 0.2s' }} onMouseOver={e => e.currentTarget.style.background = '#2575fc'} onMouseOut={e => e.currentTarget.style.background = '#6a11cb'}>Export CSV</button>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', gap: 30, flexWrap: 'wrap', marginBottom: 30 }}>
          <div style={{ background: '#fff', borderRadius: 10, boxShadow: '0 2px 8px #0001', padding: 24, flex: 2, minWidth: 320 }}>
            <div style={{ fontWeight: 600, marginBottom: 10 }}>Invoices by Status</div>
            <Pie
              data={{
          ...
          <div style={{ background: '#fff', borderRadius: 10, boxShadow: '0 2px 8px #0001', padding: 24, flex: 3, minWidth: 400 }}>
            <div style={{ fontWeight: 600, marginBottom: 10 }}>Invoices Over Time</div>
            <Bar
              data={{
                labels: months,
                datasets: [{
              ...
              options={{ plugins: { legend: { display: false } }, scales: { x: { title: { display: true, text: 'Month' } }, y: { title: { display: true, text: 'Count' }, beginAtZero: true } } }}
            />
          </div>
        </div>
        <div style={{ display: 'flex', gap: 30, flexWrap: 'wrap', marginBottom: 30 }}>
          <div style={{ background: '#fff', borderRadius: 10, boxShadow: '0 2px 8px #0001', padding: 24, flex: 2, minWidth: 320 }}>
            <div style={{ fontWeight: 600, marginBottom: 10 }}>Top Clients</div>
            <Bar
              data={{
              ...
              options={{ plugins: { legend: { display: false } }, scales: { x: { title: { display: true, text: 'Client' } }, y: { title: { display: true, text: 'Amount' }, beginAtZero: true } } }}
            />
          </div>
          <div style={{ background: '#fff', borderRadius: 10, boxShadow: '0 2px 8px #0001', padding: 24, flex: 2, minWidth: 320 }}>
            <div style={{ fontWeight: 600, marginBottom: 10 }}>Outstanding by Client</div>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ borderBottom: '2px solid #e9ecef' }}>
                  <th style={{ padding: 10, textAlign: 'left', fontWeight: 600, color: '#333' }}>Client</th>
                  <th style={{ padding: 10, textAlign: 'left', fontWeight: 600, color: '#333' }}>Outstanding</th>
                </tr>
              </thead>
              <tbody>
                {Object.entries(outstandingByClient).length === 0 ? (
                  <tr><td colSpan={2} style={{ textAlign: 'center', padding: 20 }}>No outstanding invoices.</td></tr>
                ) : (
